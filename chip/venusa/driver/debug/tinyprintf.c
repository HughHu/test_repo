...
#if defined(SMP_CPU_CNT) && (SMP_CPU_CNT > 1)
    // a spin lock to exclusivly access the log port
    // static volatile uint32_t spin_lock_drv_debug_log = 0;
    // uint32_t backoff = 10;
    // do {
    //     if (__AMOSWAP_W(&spin_lock_drv_debug_log, 1) == 0) {
    //         break;
    //     }
